...
5. 等到绿色 **可以拍了**。峰值偏弱或扫得很平时，改成黄色软成功（「这是这侧目前最清楚的，可以拍」）——仍按清晰度，不是固定角度。两种都会自动快门倒计时——需要的话点 **取消自动拍摄**。软成功拍完会说这是目前最清楚的一帧，不像耳朵正面可以点重新学习（没有绝对度数，不会强迫重学）。强峰值仍走「可以拍了」和接近最清楚的那句。然后可 **重拍** 或 **拍另一只耳**。
