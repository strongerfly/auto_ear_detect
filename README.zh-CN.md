...
1. 允许摄像头。需要的话在页头切语言。
2. 点 **拍左耳** 或 **拍右耳**（身体的左右，不是镜子）。切换左右会重置这一侧的快门/引导，另一侧记住的角度还在。拍右耳 → 向左转；拍左耳 → 向右转。
3. **转头**慢慢转（不是转电脑/手机），找到耳朵最清楚的角度。第一次要慢慢转过，系统才会记住这一侧；在此之前拍不了。
4. 等到 **可以拍了**（如果峰值偏弱，会用另一句「目前最清楚」而不是同一句 READY）。默认开着自动快门，会有倒计时——需要的话点 **取消自动拍摄**。拍完会告诉你这张是否接近最清楚的角度（没有绝对度数），然后可 **重拍** 或 **拍另一只耳**。

角度记错时用 **重新学习此侧**（再确认一次），然后慢慢转一次。转了很久仍没更清楚时会出现 **再试一次** / 重新学习 / 拨开头发换亮一点，不会只留一个灰色快门。应用里的 **使用说明** 还有转过头、摄像头权限和做不到什么。
...
| 减少闪烁 | `promptUx.minDwellMs`、`crossFamilyDwellMs`、`slowDownPreemptMs`、`smoothing.oneEuro` |
