...
5. Wait for green **Ready to capture**. A weak or flat peak uses a yellow soft state instead (“clearest we found for this ear — you can capture”), still from quality, not a fixed angle. Auto-shutter counts down for both — tap **Cancel auto-capture** if you need to stop it. After a soft shot the caption says it was the clearest frame found and that you can relearn if it doesn’t look like a frontal ear (no degrees; relearn is not forced). A strong peak stays on the Ready / near-peak lines. Then **Retake** or **Shoot the other ear**.
