...
- **SOFT_READY 相对 READY：** 弱峰值或扫过一圈分数几乎不变的启发，不是「这肯定不是耳朵」的标定。允许拍摄时界面会给出成功脉冲和「够清楚了」徽章（快门之后仍保留）——不是警告色，也不是固定偏航角。
...
- #7 文案键对齐合入
- 首启选侧合入
- soft-success 更可感知已合入（脉冲、对勾徽章、成功文案；快门之后仍保留）
- 模拟器默认收起后文案打磨（模拟器默认收起已做）

说明：1–8 只进 LIMITS、不开实现票；下一刀里「#7 文案键对齐」「首启选侧」和「soft-success 更可感知」已合入 main。剩余工程票是模拟器文案，由总管派。
