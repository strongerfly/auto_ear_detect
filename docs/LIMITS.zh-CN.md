...
引导：一条短提示，中英可切换。切换左右会重置快门、停留和引导，并显示身体左右的侧介绍；另一侧已记住的峰值保留。峰值还没锁定时只说慢慢转（不会拿先验角度喊「再转一点点 / 往回一点」）。`SLOW_DOWN`（「转慢一点」）在 yaw 猛跳时可抢占扫转提示，随后按短窗口 `slowDownHoldMs`（和 READY 的 `readyPromoteMs` 同一思路）让回 SWEEP，不会占满整段同族 `minDwellMs`。靠近已锁定峰值但还不稳定时，快门仍是灰的，不会说「保持不动」。READY 切换更快，避免提示像清单一样刷。拍完按相对峰值给反馈（没有绝对度数），可重拍或拍另一只耳。绝对角度数字只在调试开关后显示。
