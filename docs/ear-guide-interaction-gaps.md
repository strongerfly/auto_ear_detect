...
| 16 | First camera: body L/R ≠ mirror; saved stills opposite selfie | Done (`MIRROR_HINT`) |
...
| 18 | Camera permission denied empty state | Done (`CAMERA_DENIED`) |
| 19 | After capture, cue the other ear | Done (`OTHER_EAR_HINT`) |
| 20 | In-app limits hint | Done (`LIMITS_HINT`) |
| 21 | Search-window edge: keep prompting (sweep / turn-back), not silence | Done |
| 22 | i18n key structure | Chinese copy keys in `pose-config.json`; EN in README/LIMITS. Full locale files wait on PR #3 |
| 23 | MULTI_FACE | Leftover — no multi-face API from Landmarker result used; LIMITS |
